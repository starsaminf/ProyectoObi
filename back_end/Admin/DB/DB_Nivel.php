...
class DB_Nivel
{
    ...
    {
        $consulta = "SELECT * from Nivel where idOlimpiada = ? ORDER BY idNivel";
        try {
            // Preparar sentencia
    ...
    {
        $consulta = "UPDATE Nivel SET Nombre = ?, Descripcion = ?,LimiteEdad = ?,LimitePorGrupo = ?  WHERE idNivel = ? ;";
            $cmd = Database::getInstance()->getDb()->prepare($consulta);
            try {
                $cmd->execute(array($Nombre, $Descripcion, $LimiteEdad, $LimitePorGrupo,$idNivel));
                return $cmd;
            } catch (PDOException $e) {
